    ...
    <script>
        function turnPage(page) {
            if (page >= 1 && page <${pageCount}) {
                window.location.href = "/comment/loser.action?page=" + page.toString();
            }
        }
    ...
    <div class="container">
        ...
        <div style="margin-top: 30px" class="container-fluid">
            ...
            <table class="table table-hover table-responsive table-striped table-bordered">
                ...
                <tbody>
                <c:forEach var="lose" items="${loser}">
                    <tr id="${id}">
                            <%--  <td><input type="checkbox" value="1" /></td>--%>

                        <td>${lose.id}</td>
                        <td>${lose.user.id}</td>
                        <td>${lose.user.name}</td>
                        <td>${lose.title}</td>
                        <td><a id="pass" href="#" onclick="manage('${lose.id}')">审核</a>&nbsp;</td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
