...
<div class="container-fluid" style="background-color: azure">
    <div class="container">
        <div class="page_nav">
            <p>当前位置：<span>资源管理</span>/<span>已发布资源</span>
                <button id="cleanRes" class="btn btn-danger">选中撤销
                </button>
            </p>
        </div>

        <table style="text-align: center" class="table table-bordered table-responsive table-hover table-striped">
            <thead>
            <tr>
            ...
            <c:forEach var="resources" items="${resourcesAll}">
                <tr style="text-align: center" id="tr_${resources.id}">
                    ...
                    <td><a id="${resources.id}" onclick="remove(this.id)">撤销</a></td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
...
<script>
    function turnPage(page) {
        if (page >= 1 && page <${pageCount}) {
            window.location.href = "/resourceadmin/release.action?page=" + page.toString();
        }
    }
</script>
</body>
